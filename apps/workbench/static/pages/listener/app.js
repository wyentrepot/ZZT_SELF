...
async function loadNetworkStatus() {
  ...
  try {
    renderNetworkSnapshot(await fetchNetwork(`/api/listener/network/status?${networkRequestParams()}`));
  } catch (error) {
...
async function loadNetworkAssessment() {
  ...
  setNetworkLoading(true);
  try {
    const data = await fetchNetwork(`/api/listener/network/assessment?${networkRequestParams()}`);
...
async function loadNetworkObserve() {
  ...
  try {
    const [digest, overview, beacons] = await Promise.all([
      fetchNetwork(`/api/listener/network/digest?${observeQuery({ level: "" })}`),
      fetchNetwork(`/api/listener/network/overview?${networkRequestParams()}`),
      fetchNetwork(`/api/listener/network/beacons?${networkRequestParams()}&limit=1`),
    ]);
...
async function loadObserveEvents() {
  const events = await fetchNetwork(`/api/listener/network/events?${observeQuery()}&limit=200`);
...
async function toggleObserveBrief(event, row) {
  ...
  try {
    ...
    if (!brief) {
      brief = await fetchNetwork(`/api/listener/network/events/${event.frame_id}/brief`);
