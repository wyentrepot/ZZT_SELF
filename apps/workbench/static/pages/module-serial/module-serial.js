...
(function () {
  ...
  async function refreshStatus() {
    try {
      const st = await request("/api/module-serial/module-serial/status");
  ...
  async function pollLogs() {
    for (const ch of CHANNELS) {
      try {
        const data = await request(`/api/module-serial/module-serial/logs?after=${lastSeq[ch]}&channel=${ch}`);
  ...
  async function refreshPorts(ch) {
    ...
    try {
      const data = await request("/api/module-serial/module-serial/ports");
  ...
  async function toggleSerial(ch) {
    ...
    try {
      ...
      if (isStop) {
        await request("/api/module-serial/module-serial/stop", {
          method: "POST",
  ...
  // ---------- 文件选择：先试系统原生对话框（/api/module-serial/fs/pick）----------
  async function pickFile(ch) {
    ...
    btn.textContent = "选择中…";
    try {
      const data = await request("/api/module-serial/fs/pick");
  ...
  async function pickerRoots() {
    try {
      const data = await request("/api/module-serial/fs/roots");
  ...
  async function pickerList(path) {
    ...
    try {
      const data = await request(`/api/module-serial/fs/list?path=${encodeURIComponent(path)}`);
  ...
  async function startFlash(ch) {
    ...
    try {
      await request("/api/module-serial/module-serial/flash", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function sendText() {
    ...
    request("/api/module-serial/module-serial/write_text", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  function bind() {
    ...
    picker.up.addEventListener("click", () => {
      ...
      request(`/api/module-serial/fs/list?path=${encodeURIComponent(picker.currentDir)}`)
  ...
  async function cmpScan() {
    ...
    try {
      ...
      if (cmp.source === "file") {
        ...
        data = await request(`/api/module-serial/loghooks/scan?path=${encodeURIComponent(path)}&module=${module}&limit=8000`);
      } else {
        data = await request(`/api/module-serial/loghooks/realtime?channel=${module}&limit=8000`);
      }
...
// 虚拟滚动：右侧日志全量渲染但只画可视行
  ...
  function cmpStartRealtime() {
    ...
    cmp.realtimeTimer = setInterval(() => {
      const module = cmp.module;
      request(`/api/module-serial/loghooks/realtime?channel=${module}&limit=8000`)
        .then((data) => {
          cmp.events = data.events || [];
          cmpRenderEvents();
  ...
  async function simconRefreshPorts() {
    try {
      const data = await simconFetch("/api/module-serial/simcon/ports");
  ...
  async function simconRefreshStatus() {
    try {
      const s = await simconFetch("/api/module-serial/simcon/status");
      simcon.open = s.open;
      simcon.port = s.port;
  ...
  async function simconRefreshRules() {
    ...
    try {
      const data = await simconFetch("/api/module-serial/simcon/responders");
  ...
  async function simconClose() {
    try {
      await simconFetch("/api/module-serial/simcon/close", { method: "POST" });
      await simconRefreshStatus();
    } catch (err) {
  ...
  async function simconRunTask() {
    ...
    try {
      const out = await simconFetch("/api/module-serial/simcon/verify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
