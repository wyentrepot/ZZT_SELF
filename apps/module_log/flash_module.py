...
- 自己 open 指定 COM 口 → 复用 module_log/xmodem_flash.flash() → close。
...
# 目录分层后：仓库根、apps/、libs/ 加入 sys.path（在 import 顶层包之前执行）。
# 独立脚本直接运行时 sys.path[0] 为本脚本目录，必须显式注入这些路径，
# 使 `from module_log import xmodem_flash` 及 shared.* 导入可用。
_REPO_ROOT = Path(__file__).resolve().parent.parent.parent  # module_log → apps → 根
for _d in ("", "apps", "libs"):
    _p = str(_REPO_ROOT / _d) if _d else str(_REPO_ROOT)
    if _p not in sys.path:
        sys.path.insert(0, _p)

from module_log import xmodem_flash  # noqa: E402
