"""listener/desktop.py（pywebview 内嵌窗口入口）单元测试。

覆盖 pywebview 新增部分的启动逻辑，不实际起 Uvicorn 服务、不弹真实窗口：
1. desktop 模块可导入，PORT/TITLE 常量正确
2. _start_server 用正确的 listener.app:app 字符串 + 端口（mock uvicorn）
3. main() 装了 webview 时：创建窗口 URL 指向根路径 /、调用 webview.start
4. main() 未装 webview 时：回退 webbrowser 打开 + 仍启动服务
5. listener.app:app 字符串可解析出 FastAPI（frozen 下服务启动的关键）
...
import listener.desktop as desktop
...
class StartServerTests(unittest.TestCase):
    def test_start_server_uses_app_and_port(self):
        ...
        self.assertEqual(args[0], "listener.app:app")
...
class MainWebviewBranchTests(unittest.TestCase):
    ...
    def test_main_without_webview_falls_back_to_browser(self):
        ...
        # 服务启动
        self.assertEqual(len(fake_uv.run_calls), 1)
        self.assertEqual(fake_uv.run_calls[0][1].get("port"), 8765)
        # 浏览器打开根路径 /（Timer 立即触发）
        self.assertEqual(len(fake_wb.opened), 1)
        self.assertIn("127.0.0.1:8765", fake_wb.opened[0])
        self.assertIn("/", fake_wb.opened[0])
