...
- 串口类资源用 `serial_ports.json` 里的 `mapping_id`：`cco-main`、`sta-main`、`listener`、`simcon`（Windows 实际 COM 号见该文件，随接线变化，调用方应优先用 mapping_id 而非硬编码 COM 号）。
...
8. 需要时 `POST /module-sessions/{id}/stop`、`POST /listener/stop`、`POST /simcon/close` 收尾。
