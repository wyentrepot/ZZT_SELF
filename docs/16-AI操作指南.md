...
| `listener:stop` | 停止侦听台 | `listener-main` |
| `observation:create` | 创建观察任务 | mapping_id / 会话 |
| `evidence:read` | 读 operation / Artifact / 帧索引 | 对应资源 |

### 3.3 资源（resource）说明

- 串口类资源用 `serial_ports.json` 里的 `mapping_id`：`cco-main`、`sta-main`、`listener`、`simcon`。
...
| `match.kind` | 当前仅支持 `literal`（非空、≤512 字符） |
| `window` | 仅 `live` / `start=now`；`timeout_seconds` 1–3600 |
| `context.before/after` | 0–100，命中时取前后若干行做证据 |
...
- `GET /api/ai/v1/operations/{operation_id}/cancel` — 取消观察（烧录不能取消）
...
- `GET /api/ai/v1/listener/indexes/{index_id}/frames?nid=...&query=...&offset=0&limit=100` — 帧分页
...
7. 需要时 `POST /module-sessions/{id}/stop`、`POST /listener/stop` 收尾。
