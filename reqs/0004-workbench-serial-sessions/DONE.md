...
> 状态：实现、自动化验证与真机只读/交互验证已完成；**烧录（XMODEM 写固件）未执行**（不可回退，需用户单独明确授权）。
> 日期：2026-08-20（核对/收尾）；真机验证 2026-08-20 于 WSL（root）执行，串口全部真实枚举可用。
...
环境：WSL Ubuntu-22.04 以 root 运行；真实串口全部枚举可用：`/dev/ttyUSB0`=COM4 侦听、`/dev/ttyACM0`=COM8 CCO、`/dev/ttyACM1`=COM9 STA、`/dev/ttyUSB1`=COM24 模拟集中器。验证通过直接驱动服务层/CLI 完成，未起 uvicorn，未烧录。
...
### 未执行
- **烧录（XMODEM 写固件）**：不可回退，未执行，等待用户单独授权。
...
4. **烧录（XMODEM 写固件）未执行**：不可回退，等待用户单独明确授权。
5. **模拟集中器部分步骤 fail**：`anhui_minute_collect.json` 中任务配置查询/写配置等步骤收到响应但内容断言不符（设备当前状态与任务期望值差异），待真机设备状态对齐后复查；与需求 0004 的映射/链路无关。

## 结论

需求 0004 的代码实现、自动化验证与真机只读/交互验证均已完成：统一映射 4 端口真机生效、侦听台 COM4 真实采集、模块日志 COM8/COM9 双会话并行采集、串口独占冲突检测、模拟集中器 COM24 收发链路全通。文档收尾（TODO 勾选、DONE.md、docs 波特率修正）已完成并推送远程。剩余为烧录真机动作（需单独授权）与上述 4 项待办，均不阻塞代码合入。
