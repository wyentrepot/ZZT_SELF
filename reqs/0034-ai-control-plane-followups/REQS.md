...
> 状态：🚧 进行中（P0 工作台使用优化实施中，P1/P2 暂缓）｜ 当前基线：v1.1（2026-09-21）
> 创建：2026-09-21
...
### 变更 2 ｜ 2026-09-21 ｜ 用户
- **改成什么**: 执行范围收口——当前**仅实施「工作台使用优化」部分（BR-1/BR-2/BR-3，即 PLAN P0）**；
  BR-4（mclt 工具三改进，涉及 CCO 日志解析与 06F230 集中器侧分布）与 BR-5（README 首屏）**暂缓**，另行排期。
  需求正文 BR-1~BR-5 内容不变，仍为最终交付范围。
- **为什么**: 用户明确"先只添加工作台使用优化的补丁部分，暂时不改日志以及集中器等部分的功能"。
- **影响**: 执行顺序与当前范围——PLAN.md 标注 P0 先行、P1/P2 暂缓；TODO.md 阶段状态同步；
  本次提交仅含 apps/workbench（run.py/ai_v2_api/ai_contracts/ai_capability_service + 单测）；
  tools/taiti/分钟采集 与 README.md 不在本次改动内。
- **变更前基线**: v1.0
- **变更后基线**: v1.1
- **被取代**: 无（范围/顺序收口，非需求内容变更）。
