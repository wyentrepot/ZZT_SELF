# REQS-0034 — PLAN（依据 REQS.md 基线 v1.0，2026-09-21）
...
- [ ] 文件：`apps/workbench/run.py`
- [ ] 现状（前置检查）：`run.py` 无环境变量检测横幅；`ai_v2_api.py:31-38` 401 文案为「缺少 Bearer token」
- [ ] 实现：`__main__` 块在 `_ensure_serial_nodes()` 后打印前置检查清单（纯 print，不动逻辑）：
  ```
  ...
  ```
- [ ] 验证：`python3 apps/workbench/run.py`（或 import 冒烟）输出含三行前置检查（不依赖启动成功）

### T-P0.2 BR-1 401 文案区分（RED → 实现 → GREEN）
- [ ] 文件：`apps/workbench/ai_v2_api.py` `_bearer_grant`（:30-38）
- [ ] RED：新增用例——无 Authorization 且未开 local_full 时，断言 detail 含「WORKBENCH_LOCAL_FULL_ACCESS」
- [ ] 实现：`_bearer_grant` 的"缺少 Bearer token"分支文案改为：
  `缺少 Bearer token；本机 loopback 可在启动时设 WORKBENCH_LOCAL_FULL_ACCESS=1 免 token`
  （无效/过期/撤销分支文案不变）
- [ ] GREEN：新用例 1 passed；`test_v2_loopback_without_flag_requires_a_bearer_grant` 仍通过
  （其断言是 status_code 401 + code 字段，不含 detail 精确匹配——实现前确认，若匹配则同步改断言）

### T-P0.3 BR-2 investigation 响应"必读 job"提示（RED → 实现 → GREEN）
- [ ] 文件：`apps/workbench/ai_contracts.py`（JobEnvelope）+ `apps/workbench/ai_capability_service.py`（_envelope）
- [ ] 决策：**首选显式字段** `follow_up: str | None = None`（JobEnvelope）——语义清晰、契约自描述；
  `_envelope` 中 investigation 且非终态时填 `"GET /api/ai/v2/jobs/{job_id}"`
- [ ] RED：新增用例——investigation 创建响应 `follow_up` 含 `/jobs/` 与 job_id；非 investigation 为 null
- [ ] GREEN：新用例通过；既有 investigation 用例（信封/evidence）不破坏

### T-P0.4 BR-3 L3 ref 格式错误带示例（RED → 实现 → GREEN）
- [ ] 文件：`apps/workbench/ai_capability_service.py` `_l3_items`（:506-508）
- [ ] RED：新增用例——`ref=bad` 的 evidence 请求 422，detail 含 `listener:<index_id>:<frame_id>`
- [ ] 实现：格式错误分支文案改为：
  `L3 ref 格式错误：{ref_text}；合法格式 listener:<index_id>:<frame_id>，如 listener:idx-xxx:123`
- [ ] GREEN：新用例通过；既有 evidence 用例不破坏
