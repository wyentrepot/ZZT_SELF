...
## 阶段 P2b · 第二步：侦听台/模块日志平移（变更 2 纳入）
- [x] 侦听台：hero 头部+页签行 → 单行二级页签条（模块名+6视图+DLL芯片），筛选条恒定其下，元素 id 全保留
- [x] 侦听台 Dock 钩子：索引完成/失败、串口采集启动/增量、单帧调试成功
- [x] 模块日志：头部+ms-tabs → 单行页签条（模块名+2页签+服务状态）
- [x] 模块日志 Dock 钩子：增量 RX/TX 行 → 「日志流」（EVENT 不推）
- [x] 浏览器实测（无后端口径）：两页页签条渲染、模块切换、既有面板/会话结构完整
- [ ] 带后端复验两页 Dock 推送真数据
