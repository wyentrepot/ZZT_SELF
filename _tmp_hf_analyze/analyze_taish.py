...
# 超时/补抄上限
MAX_RE = re.compile(r"read cycle reach max|read fail|cycle")
...
def _decode_addr_from_frame(hex_str: str) -> Optional[str]:
    """从采集帧 hex 中直接提取目标表地址（复用 frame_codec 解析）。"""
    try:
    ...
    if d.get("afn") != 0x63:
        return None
    buff = d.get("buff", b"")
    # buff 布局（经实测）：900000 <2B序号> <6B地址> f101 ...
    # 更稳健：在 buff 中找 6 字节小端地址模式 xx xx xx 01 xx xx（地址域）
    m = re.search(rb"([0-9A-Fa-f]{12})", buff.hex().encode())
    if m:
        # buff.hex() 是连续 hex；地址位于 '900000<seq4hex><12hex>' 后
        h = buff.hex()
        # 找 f101 前的 12 位 hex 作为地址
        fm = re.search(r"([0-9A-Fa-f]{12})f101", h)
        if fm:
...
def main() -> None:
    if len(sys.argv) < 2:
        print("用法: python3 analyze_taish.py <台体日志路径>")
        sys.exit(1)
    res = analyze_taish_log(sys.argv[1])
