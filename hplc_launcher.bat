...
powershell -NoProfile -Command "try { $version = Invoke-RestMethod -UseBasicParsing 'http://127.0.0.1:8765/api/version' -TimeoutSec 2; $api = Invoke-RestMethod -UseBasicParsing 'http://127.0.0.1:8765/openapi.json' -TimeoutSec 2; if ($version.picker_api_revision -eq 2 -and $version.minute_analysis_api_revision -eq 2 -and $null -ne $api.paths.'/api/fs/pick' -and $null -ne $api.paths.'/api/logs/minute-analysis') { exit 0 } else { exit 1 } } catch { exit 1 }" >nul 2>&1
