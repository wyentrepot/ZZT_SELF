...
call :build tools\packaging\hplc_parser.spec
if errorlevel 1 goto :failed
echo [3/3] 完成。
echo 输出: %CD%\dist\侦听台\
echo 冒烟: .venv\Scripts\python.exe tools\scripts\smoke_test_packaged.py
exit /b 0
...
call :build tools\packaging\hplc_parser_desktop.spec
if errorlevel 1 goto :failed
echo [3/3] 完成。
...
call :build tools\packaging\module_log.spec
if errorlevel 1 goto :failed
echo [3/3] 完成。
...
:build
set "SPEC=%~1"
".venv\Scripts\python.exe" "tools\packaging\check_encrypted.py" "apps\module_log\desktop.py"
...
if not exist "apps\module_log\desktop.py" (
    popd
    echo [ERROR] 解压明文副本失败（未找到 apps\module_log\desktop.py）。
    exit /b 1
)
