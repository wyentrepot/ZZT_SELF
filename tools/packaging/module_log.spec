...
在仓库根目录运行：
    .venv\Scripts\python.exe -m PyInstaller --clean --noconfirm tools\packaging\module_log.spec

datas 布局与 module_log/app.py 的 frozen 路径解析一致：
- static/ 落入 _MEIPASS（onedir 下即 _internal/）
- 运行时数据 LOG/、runtime/ 不入包，由 exe 在自身目录生成（frozen 下日志仍落 exe 同目录 LOG/）
"""
...
a = Analysis(
    [str(ROOT / "apps" / "module_log" / "desktop.py")],
