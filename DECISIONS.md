...
## ADR-37 网络承载评估 A 档指标：关联请求/代理变更频率（国网通用）

- **日期**：2026-08-25
- **状态**：✅ 生效
- **决定**：网络承载能力评估从 2 维扩展为多维，新增 A 档「稳定性维度」：**关联请求频率 + 代理变更频率**（国网通用，非安徽专属），并明确用户拍板的判定规格：
  - **数据源**：`simple.FrmType`（C# DLL 已输出 `"关联请求"`(0x0030)、`"代理变更请求"`(0x0031)），无需新增解析。
  - **代理变更判级**：`proxy_change_ratio = 代理变更帧数 / 该周期总帧数 × 100%`；**>8% → 该周期稳定性降级**。
  - **关联请求判级**：按记忆库 A2 规则，关联请求占比 >5%（默认值，可调）或相对基线突增 → 降级。
  - **时间门槛（用户规格）**：稳定性维度（代理变更/关联请求频率判级）**仅当日志总时长 >2h（7200s）才启用**；短日志只统计不判级，返回 `enabled:false, reason:"log_too_short"`。
  - **合并规则**：稳定性维度评级与通信成功率/离线率取**最差**作为该周期评级；汇总仍为任一故障即故障。
