"""模块日志/烧录串口独立应用（需求 0002 拆分）。

与侦听台（listener_app）完全独立：独立 FastAPI 应用、独立端口（默认 8766）、
...
from hplc_web import app as _shared_app  # 复用辅助函数与常量
...
def create_module_serial_app(module_serial_service=None) -> FastAPI:
    ...
    class ModuleSerialStartRequest(BaseModel):
        ...
        log_type: str = Field("cco", pattern=r"^(cco|sta)$")  # 模块日志归属 cco/sta
    ...
    def module_serial_start(request: ModuleSerialStartRequest):
        ...
        try:
            return module_serial_service.start(
                ...
                stopbits=request.stopbits, log_type=request.log_type,
            )
    ...
    # ---- 固件选择（复用侦听台 fs API，仅 pick 选固件路径）----
...
app = create_module_serial_app(module_serial_service=module_serial_svc)
